...
if use_ramp
    ...
    notch_data = read_notch_bin('samples/arthur.input',6400);
    noise_data = read_notch_bin('samples/arthur.noise',6400);
    Ns = size(notch_data,1);
    fprintf('Ns = %d\n', Ns);
end

weights = importdata('samples/arthur.weights');
weights = weights(:,3);
...
for ic = 1:Ns
    ...
    for jc = 1:{Nchan}
        ...
        [drift, have_lock, foutreal1, foutimag1, foutreal2, foutimag2, foutreal3, foutimag3, foutreal4, foutimag4, corout1, corout2, corout3, corout4,  fout_ready ] = ...
